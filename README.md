...
see `theatre/README.md`
or [docs](https://concerto-app.github.io/theatre).
...
(base) $ git clone https://github.com/concerto-app/theatre
(base) $ cd theatre
(base) $ conda env create -f environment.yml
(base) $ conda activate theatre
(theatre) $ cd theatre
(theatre) $ poetry install --extras dev
(theatre) $ theatre
```
...
into `theatre` directory and run:
...
write the tests and put them in `theatre/tests`.
...
Docs should be placed in `theatre/docs/docs`. They
are pretty straightforward to write.

To build the docs,
`cd` into `theatre/docs` and run:
...
```sh
 docker build -t theatre .
```

To also run the container in one go, run:

```sh
docker build -t theatre . && docker run --rm -it theatre
```
